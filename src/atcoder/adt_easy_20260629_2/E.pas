...
var
	q, i, x, c, n: int32;
	tp: int8;
	sl: TSortedList<int32>;
	InputBuf, OutputBuf: array [1 .. 65536] of Char;

function slCompare(constref Left, Right: int32): Integer;
begin
...
begin
	...
	readln(q);
	sl := TSortedList<int32>.Create(TComparer<int32>.Construct(slCompare));
	...
	for i := 1 to q do begin
		...
		case tp of
			...
			2: begin
				read(x, c);
				c := min(c, sl.BisectRight(x) - sl.BisectLeft(x));
				while c > 0 do begin
					sl.Discard(x);
					dec(c);
				end;
