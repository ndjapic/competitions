...
var
	x, q, i, a, b: int32;
	MinHeap, MaxHeap: THeap<int32>;
	InputBuf, OutputBuf: array [1 .. 65536] of Char;

function MinHeapCompare(constref Left, Right: int32): Integer;
begin
	Result := Left - Right;
end;

function MaxHeapCompare(constref Left, Right: int32): Integer;
begin
	Result := - Left + Right;
end;
...
begin
	...
	MinHeap := THeap<int32>.Create(TComparer<int32>.Construct(MinHeapCompare));
	MaxHeap := THeap<int32>.Create(TComparer<int32>.Construct(MaxHeapCompare));
	...
	for i := 1 to q do begin
		...
		MaxHeap.Push(MinHeap.Pop);
		MaxHeap.Push(MinHeap.Pop);
		while MaxHeap.Count > MinHeap.Count do MinHeap.Push(MaxHeap.Pop);
		x := MinHeap.Pop;
		writeln(x);
	end;
